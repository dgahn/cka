...
21) resource requests/limits 조정
  - requests/limits 변경
22) QoS class 변경 확인
  - BestEffort/Burstable/Guaranteed 확인
23) huge requests로 Pending 유도
  - requests를 노드 용량 이상으로 설정
24) sidecar 영향 확인
  - sidecar 컨테이너 추가 후 배치 확인
25) StatefulSet 분산 정책 확인
  - podManagementPolicy, ordered vs parallel 확인
26) DaemonSet 스케줄링 특성 확인
  - 노드당 1개 유지 확인
27) nodeSelectorTerms 여러 개 지정
  - matchExpressions 여러 항목 추가
28) matchExpressions 사용
  - In, NotIn, Exists 실습
29) preferredDuringScheduling 테스트
  - preferred 설정 후 분포 확인
30) Day10 리소스 정리
  - k delete pod,pdb,priorityclass --all
