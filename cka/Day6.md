...
21) ClusterRoleBinding 잘못 연결 후 권한 과다 확인
  - ClusterRoleBinding 제거 후 권한 회수 확인
22) RBAC 오타로 Forbidden 발생 유도
  - 존재하지 않는 Role/SA로 바인딩
...
26) --as로 impersonation 테스트
  - k get pods --as=system:serviceaccount:default:app-sa
27) --as-group 테스트
  - k get pods --as=alice --as-group=devs
28) TokenRequest 생성(가능한 버전)
  - k create token app-sa
29) RBAC 리소스 정리
  - k delete role,rolebinding,clusterrole,clusterrolebinding --all
30) Day6 리소스 전체 삭제
  - k delete sa --all
